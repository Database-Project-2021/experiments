...
public abstract class TPartStoredProcedure<H extends StoredProcedureParamHelper>
	...
	public SpResultSet execute() {
		try {
			...
			Timer.getLocalTimer().startComponentTimer("tx commit");
			tx.commit();
			...
			if(tx.isReadOnly())
				Timer.getLocalTimer().stopComponentTimer("Average Time Of Read-Only Txn");
			else
				Timer.getLocalTimer().stopComponentTimer("Average Time Of R/W Txn");
